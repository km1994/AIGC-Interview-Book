...
![](img/ScreenShot_2026-09-18_182434_991.png)
...
### 第二部分：进阶篇（41-95题）
#### [模块四：索引与向量数据库（41-55题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### [模块五：检索优化技术（56-75题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### [模块六：生成与上下文融合（76-90题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### 大模型RAG 模块七：[评估与可观测性（91-95题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### 第三部分：生产实战篇（96-140题）
#### 模块八：[工程部署与性能优化（96-110题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### 模块九：[多模态与结构化数据RAG（111-125题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### 模块十：[前沿变体与高阶技巧（126-140题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### 模块十一：[经典业务场景设计（141-150题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### 模块十二：[开放思路与面试官追问（151-160题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### [模块三：规划与执行](https://github.com/km1994/AIGC-Interview-Book/tree/main)（25-40题）
...
#### [模块四：工具系统设计（41-55题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### [模块五：多Agent协作（56-70题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### [模块六：生产环境挑战（71-80题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### [模块七：经典场景与案例（81-95题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### [模块八：思路题与开放题（96-100题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### 模块四：[GRPO 深度解析（41-50题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### 模块五：[KTO 与 Rejection Sampling（51-58题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### 模块六：[RLVR 与新兴方向（59-65题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### 模块七：[方法对比与选型（66-75题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### 模块八：[数据与评估（76-85题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### 模块九：[工程实践（86-90题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### 模块十：[经典场景与案例分析（91-105题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### 模块十一：[前沿与开放题（106-120题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### 模块一：[Loop Engineering 基础概念（1-15题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
- **回答**：ReAct（Reasoning + Acting）出自 2022 年 Google 的论文，本质是一个 **Prompt 范式**——让模型按「思考→行动→观察」的格式输出。它的全部「魔法」在于用一段提示词诱导模型按格式输出，外面再写个循环把观察结果拼回上下文。而 Loop Engineering 是一套**系统架构**，不仅包含 ReAct 的推理-行动循环，还包含自动化触发、并行隔离、技能沉淀、工具连接、子 Agent 分工、跨会话记忆等完整的基础设施。可以把 ReAct 理解为 Loop 内部的一个执行模式，而 Loop Engineering 是让这个模式能够长期、稳定、规模化运行的系统工程。
...
- **回答**：自动化是 Loop 的「心跳」或「定时任务」，让 Loop 真正成为一个自行运转的系统而非一次性执行。常见触发方式包括：定时触发（如 cron 调度）、事件触发（如 GitHub hook）、持续目标模式（`/goal`——一直运行到目标完成为止）。没有自动化，Loop 就只是「手动跑了一次」，不叫循环。

**8. 【⭐】Loop Engineering 中的「Skill」是什么？**

- **考察点**：对知识沉淀机制的理解
- **得分项**：能解释 Skill 的定义、能说明其作用和价值、能举例
- **回答**：Skill 是把项目相关的知识、规范、架构、踩过的坑等打包成一份文件，让 Agent 每次开工前先读取。它解决了「每次从零推导」的问题——没有 Skill，Loop 每一轮都要从零理解项目该怎么做；有了 Skill，每一轮都能吸取上一次的经验。Skill 本质上是一种**上下文缓存机制**，让项目知识在 Loop 的各轮之间得到沉淀和复用。
...
- **回答**：Harness（挽具/治理框架）是围绕模型运行的整套脚手架——测试工具、输入输出约束、格式校验、评估框架。它的核心是「怎么判断答得对不对」。Harness Engineering 关注的是单次执行的质量控制，而 Loop Engineering 关注的是**多轮持续执行的整体治理**——不仅要判断单次对不对，还要决定不对之后怎么办、怎么恢复、怎么终止。
...
- **回答**：个人使用 AI 编程关注的是**效率**——一天写出更多代码、一个周末做出一个产品。而企业关注的是**可控性**——交付链路是否可控、风险是否可追溯、权限是否清晰、知识是否沉淀、成本是否可预算。一名工程师用 AI 提效 30%，不必然意味着团队交付效率提升 30%——更多代码可能带来更重的 Review 压力。Loop Engineering 本质上是在讨论一个更大的问题：当 Agent 具备持续循环执行任务的能力后，组织是否有能力重新设计自己的生产流程。

**15. 【⭐】Loop Engineering 的「记忆层」解决什么问题？**

- **考察点**：对跨会话记忆机制的理解
- **得分项**：能说明记忆层的作用、能理解长任务 Agent 的「失忆」问题
- **回答**：记忆层记录「做过什么、现在卡在哪、下一步是什么」。它的作用很朴素但关键——长任务 Agent 如果没有记忆层，很快就会开始「失忆」。记忆层可以是一个 Markdown 文件、一个 Linear Board 或其他状态记录系统。它让 Loop 的各轮之间有了**状态的连续性**，而不是每轮从零开始。


#### 模块二：[循环控制与终止机制（16-30题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
- **回答**：不能只返回简单的「Error」，而应该返回**结构化的错误信息**，包含错误类型、原因和可操作的建议。例如：`{"status": "failed", "error_type": "Timeout", "retry_after": 5}`。如果是 SQL 报错，要把报错堆栈中最关键的信息塞回上下文，让 Agent 像程序员修 Bug 一样根据报错信息修正下一步。
...
- **回答**：收敛判断是评估「任务是否已经完成」或「是否在朝着正确方向前进」的机制。实现方式包括：①**规则检查**——如测试全部通过、文件已修改、PR 已创建；②**独立评估 Agent**——用一个独立的 Agent（甚至不同模型）在每轮结束时检查目标是否达成；③**阈值判断**——如代码覆盖率提升到 X%、性能指标达到 Y。Claude Code 和 Codex 的 `/goal` 功能就是这种模式——每跑完一轮，由独立 Agent 判断是否完成。
...
- **回答**：断点续传依赖**状态持久化**——把每轮结束时的状态（已完成步骤、当前上下文、中间产物）写入持久存储（如文件、数据库）。当 Loop 因故中断后，重启时从持久化存储中读取最后保存的状态，从中断处继续，而不是从头开始。Codex CLI 的 `/goal` 功能支持「跨会话保留状态」。
...
- **回答**：触发器类型包括：①**定时触发**——cron 调度，适合周期性任务（如每日报告）；②**事件触发**——GitHub hook、Webhook，适合响应外部事件；③**持续目标模式（`/goal`）** ——一直运行到目标完成为止；④**手动触发**——用户主动启动，适合需要人工判断的场景。


#### 模块三：[工具与集成（31-40题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
- **回答**：Function Calling 是大模型调用工具的一种**接口方式**——模型输出结构化的函数调用请求。MCP（Model Context Protocol）是一个**协议标准**，定义了 Agent 与外部工具之间如何发现、连接和交互。可以这样理解：Function Calling 是「怎么调用」，MCP 是「调用谁、怎么发现、怎么标准化」——MCP 在 Function Calling 之上提供了服务发现和标准化的能力。
...
- **回答**：Prompt 是一次性的指令——每次使用都要重新输入或重新构造。Skill 是**可复用的知识包**——把项目技术栈、构建方式、踩坑经验等打包成一份文件，Agent 每次开工先读。区别在于：Prompt 是「这次怎么说」，Skill 是「这个项目长期怎么干」。Skill 让 Loop 每一轮都能吸取上一次的经验，而不是从零推导。


#### 模块四：[监控与可观测性（41-50题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)

**41. 【⭐】Loop 的可观测性面临哪些挑战？**

- **考察点**：对 Loop 监控难点的理解
- **得分项**：能识别 Loop 可观测性的特殊性、能理解与传统监控的差异
- **回答**：传统 APM 监控的是**确定性请求链路**，而 Loop 的链路是**动态生成的、非确定性的、跨多个 Agent 的**。挑战包括：①**链路不可预测**——每次执行路径不同；②**跨 Agent 追踪**——多个子 Agent 之间的调用关系复杂；③**状态难以重建**——Loop 跑了 47 轮后出了问题，很难回溯当时的状态；④**缺乏成熟方案**——当下没有成熟的 Loop Observability 方案。
...
#### 模块五：[架构设计与系统集成（51-65题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
- **回答**：规划器-执行器模式把流程拆成两个角色：**规划器**一次性把任务分解成完整的步骤清单（Plan）；**执行器**按清单逐条执行，不再反复调用大模型做全局推理。这种模式解决了 ReAct「走一步想一步」的两个问题：①**短视**——容易陷入局部最优；②**昂贵**——每一步都要带着全部历史调一次大模型。缺点是规划器需要很强的全局推理能力，且 Plan 可能因环境变化而过时。

**54. 【⭐⭐】什么是「评估者-优化者」模式？在 Loop 中如何应用？**

- **考察点**：对 Agent 协作模式的理解
- **得分项**：能解释评估者-优化者模式、能说明其在 Loop 中的应用
- **回答**：评估者-优化者模式是 Anthropic 在《Building Effective Agents》中提出的——一个 Agent 生成方案，另一个 Agent 评估并给出改进建议，循环直到达标。在 Loop 中，这个模式体现为**子 Agent 分工**：写代码的 Agent 是「优化者」，检查代码的 Agent 是「评估者」。评估者给出反馈，优化者根据反馈修改，循环直到评估通过。
...
- **回答**：RAG 是从外部知识库**检索**相关信息，解决「模型不知道」的问题。记忆层是**记录** Loop 运行过程中的状态和进度，解决「模型忘记了」的问题。区别在于：RAG 是「查资料」，记忆层是「记笔记」。两者可以配合——记忆层记录状态，RAG 在需要时检索相关知识来补充上下文。
...
- **回答**：跨会话记忆通过**持久化存储**实现——把 Loop 的状态、进度、经验写入持久存储（文件、数据库、Linear Board 等）。Codex CLI 的 `/goal` 功能支持「跨会话保留状态」——设定一个持久目标后，即使关闭会话再打开，Loop 也能从上次中断的地方继续。跨会话记忆让 Loop 从「一次性运行」变成了「长期运行」。
...
#### 模块六：[性能与优化（66-75题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### 模块七：[生产环境挑战（76-80题）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
- **回答**：不会取代，而是**叠加**。Prompt Engineering 没死，它是地基——一个 Loop 本来就是由一堆 Prompt 组成的。Prompt 写得烂，扔进 Loop 里只会让烂活儿产出得更快。AI 工程的规律是**分层沉淀**：新范式不意味着旧范式消亡，而是整个技术栈的丰富和深化。工程师需要同时掌握 Prompt Engineering（单次交互质量）和 Loop Engineering（系统持续运行）。

**99. 【⭐⭐⭐】Loop Engineering 和「Agentic Engineering」是什么关系？**

- **考察点**：对 AI 工程最新概念的辨析
- **得分项**：能区分两个概念、能理解它们关注的侧重点
- **回答**：Agentic Engineering（智能体工程）关注的是**让 AI Agent 自主行动的系统工程能力**——包括工具调用、规划、记忆、异常处理等。Loop Engineering 是 Agentic Engineering 的一个**核心子集**——聚焦于「循环执行」这个维度。Agentic Engineering 的范围更广，还包括 Agent 的注册发现、任务分发、A2A 协议、状态共享等。Loop Engineering 回答的是「如何让 Agent 持续运行不出错」，Agentic Engineering 回答的是「如何构建一个完整的 Agent 系统」。

**100. 【⭐⭐⭐】如果 Loop 在凌晨 3 点跑了 47 轮输出了一堆问题代码，你怎么 Debug？**

- **考察点**：对生产环境故障排查的综合能力
- **得分项**：能提出系统的 Debug 方法论、能理解 Loop Debug 的特殊性
- **回答**：这是 Loop 可观测性的经典难题。Debug 步骤：①**状态重建**——从持久化存储中恢复第 47 轮的完整状态快照；②**逐轮回放**——从第 1 轮开始逐轮回放，找到问题引入的轮次；③**差异分析**——对比第 N 轮和第 N-1 轮的状态差异，找出变化点；④**输入重放**——用相同的输入重新跑有问题的轮次，确认是否可复现；⑤**日志分析**——查看结构化日志，分析每轮的 Thought/Action/Observation；⑥**人工介入**——如果无法定位，保存完整状态并通知人工。根本解决之道是**预防**——更好的可观测性设计、更完善的异常检测、更早的熔断机制。
...
### [Qwen3-RL训练过程详解](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
- [DeepSeek-R1 百问百搭-DeepSeek-R1篇](https://github.com/km1994/AIGC-Interview-Book/tree/main) :fire:
...
### [从 DeepSeek-V3 到 Kimi K2：八种现代大语言模型架构设计](https://github.com/km1994/AIGC-Interview-Book/tree/main):fire: 
...
- [点击查看答案](https://github.com/km1994/AIGC-Interview-Book/tree/main)

### [LLMs 对比篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [百川智能baichuan7B、13B、53B、baichuan2 总结篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [LLaMa 篇](https://github.com/km1994/AIGC-Interview-Book/tree/main) 

- 一、相比较于llama而言，llama2有哪些改进，对于llama2是应该如何finetune？

- [点击查看答案](https://github.com/km1994/AIGC-Interview-Book/tree/main)

### [GPT 经验篇](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [beam search及其变种](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [Continuous Batching 与 Selective Batching 实现](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [大模型(LLM)部署框架对比篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [大模型（LLMs）推理加速篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [大模型（LLMs）加速篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [LLMs 推理性能面](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [LLM（大语言模型）部署加速方法——PagedAttention篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [大模型推理加速工具 —— vLLM](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [LLM（大语言模型）部署加速方法——Faster Transformer篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [纯Python超轻量高性能LLM推理框架 —— LightLLM](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [LLM推理技术之StreamingLLM：如何拥有无限长生成能力](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [SwiftInfer —— 大模型无限流式输入推理飙升46%，打破多轮对话长度限制](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
- [从头开始了解 LLM 中的 KV 缓存并对其进行编码](https://github.com/km1994/AIGC-Interview-Book/tree/main)
  - 引言
  - 一、什么是 KV 缓存？
...
- [大模型推理加速——KV Cache篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
  - 一、介绍一下 KV Cache是啥？
  - 二、为什么要进行 KV Cache？
    - 2.1 不使用 KV Cache 场景
    - 2.2 使用 KV Cache 场景
...
- [KV-Cache 面试参考题篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
  - 一、为什么文本生成如此缓慢?
  - 二、如何解决文本生成缓慢问题？
  - 三、什么是键值缓存？
  - 四、KV缓存如何加速？
...
- [从多头共享到潜变量：MLA在低秩投影与按需解压中重新定义 KV-Cache 存储](https://github.com/km1994/AIGC-Interview-Book/tree/main)
  - 前言
  - 一、为什么要减少 KV-Cache？
    - 1.1 长序列推理中显存的“隐形杀手”
    - 1.2 显存与带宽的双重约束
...
### [大模型（LLMs）基础面](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [Layer normalization 篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [LLMs 激活函数篇](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [Attention 升级面](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
- [跨注意力机制（Cross-Attention）篇](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [transformers 操作篇](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [LLMs 损失函数篇](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [相似度函数篇](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
## [二、大模型（LLMs）进阶面](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [大模型（LLMs）微调面](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [大模型 SFT Trick 篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [大模型（LLMs）训练经验帖](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [大模型（LLMs）langchain 面](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [多轮对话中让AI保持长期记忆的8种优化方式篇](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [基于langchain RAG问答应用实战](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
#### [基于LLM+向量库的文档对话 经验面](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### [RAG（Retrieval-Augmented Generation）面](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [大模型（LLMs）RAG —— pdf解析关键问题](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [大模型（LLMs）RAG 版面分析——表格识别方法篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [大模型（LLMs）RAG 版面分析——文本分块面](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### [大模型外挂知识库优化——如何利用大模型辅助召回？](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### [大模型外挂知识库优化——负样本样本挖掘篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### [RAG（Retrieval-Augmented Generation）评测面](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### [检索增强生成(RAG) 优化策略篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### [RAG 关键痛点及对应解决方案](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### [大模型（LLMs）RAG 优化策略 —— RAG-Fusion篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
#### [Graph RAG（Retrieval-Augmented Generation） 面 —— 一种 基于知识图谱的大模型检索增强实现策略](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [大模型（LLMs）参数高效微调(PEFT) 面](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [配器微调（Adapter-tuning）篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [提示学习（Prompting）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [LoRA 系列篇](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [如何使用 PEFT库 中 LoRA？](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [大模型 SFT 方式对比篇](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [大模型（LLMs）推理面](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [从0到1：揭秘LLM预训练前的海量数据清洗全流程](https://github.com/km1994/AIGC-Interview-Book/tree/main) :fire:
...
### [大模型（LLMs）增量预训练篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [增量预训练（Pretrain）样本拼接篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [基于lora的llama2二次预训练](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
## [九、大模型（LLMs）评测面](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [大模型（LLMs）强化学习面](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [大模型（LLMs）强化学习——RLHF及其变种面](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [大模型（LLMs）强化学习—— PPO 面](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [RLHF平替算法DPO篇](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [reward 篇](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [强化学习在自然语言处理下的应用篇](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [大模型（LLMs）训练集面](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [大模型（LLMs）LLM生成SFT数据方法面](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [大模型（LLMs）显存问题面](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [大模型（LLMs）显存优化策略篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [大模型（LLMs）分布式训练面](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [图解分布式训练（一） —— 流水线并行（Pipeline Parallelism）面](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [图解分布式训练（二） —— nn.DataParallel面](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [图解分布式训练（三） ——  nn.parallel.DistributedDataParallel](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [图解分布式训练（四） ——  torch.multiprocessing 详细解析](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [图解分布式训练（五） ——  AMP混合精度训练 详细解析](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [图解分布式训练（六） —— Pytorch的 DeepSpeed 详细解析](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [图解分布式训练（七）—— accelerate 分布式训练 详细解析](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [图解分布式训练（八）—— ZeRO 学习](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [大模型分布式训练故障恢复篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [图解分布式训练（九）—— Megatron-LM 篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [分布式训练 Trick 汇总篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [pytorch 分布式计算 坑/bug 梳理篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [大模型（LLMs）agent 面](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
- [函数调用 Function Call 篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
- [开源模型 Function Call 篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
## [十五、LLMs 位置编码篇](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [LLMs Tokenizer 篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [怎么让英文大语言模型支持中文？（一） —— 构建中文tokenization](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [怎么让英文大语言模型支持中文？（二） —— 继续预训练篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [怎么让英文大语言模型支持中文？（三） —— 对预训练模型进行指令微调](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [大模型幻觉（LLM Hallucination）面](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [大模型的幻觉问题篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [如何缓解大模型幻觉？](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [思维链 Chain-of-Thought（COT）篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [思维链 Chain-of-Thought（COT）变体篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [小样本提示学习篇](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
## [二十一、LLMs 测试集 中 数据泄露 问题篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
## [二十二、MOE（Mixture-of-Experts）篇](https://github.com/km1994/AIGC-Interview-Book/tree/main) :fire:

### 22.1 [MOE（Mixture-of-Experts）篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### 22.2 [MOE大模型对比篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### 22.3 [MoE 大模型负载均衡策略演进的回顾](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [大模型知识蒸馏技术方法（DeepSeek， Llama 4 \& Gemma 3中使用的技术）](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [大模型蒸馏篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [LLMs 浮点数篇](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [自定义 CUDA 函数的轻量级包装器 —— bitsandbytes篇](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
## [二十四、大模型（LLMs）软硬件配置面](https://github.com/km1994/AIGC-Interview-Book/tree/main)

1. 建议的软件环境是什么？

- [点击查看答案](https://github.com/km1994/AIGC-Interview-Book/tree/main)

## [二十五、Token及模型参数准备篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [多模态常见面试篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [NLP Trick 篇](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [文本分类常见面试篇](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [文本摘要常见面试篇](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [命名实体识别常见面试篇](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [向量检索常见面试篇](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [LLMs 其他 Trick](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [大模型——角色扮演大模型篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
- [kimi1.5 论文研读](https://github.com/km1994/AIGC-Interview-Book/tree/main) 
...
### [OpenAI o1 面试篇](https://github.com/km1994/AIGC-Interview-Book/tree/main)
...
### [Scaling LLM Test-Time：谁说类o1推理一定要用RL?](https://github.com/km1994/AIGC-Interview-Book/tree/main)
