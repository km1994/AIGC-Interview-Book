...
- [LLMs 损失函数篇](#llms-损失函数篇)
  - [一、介绍一下 KL 散度？](#一介绍一下-kl-散度)
  - [二、交叉熵损失函数写一下，物理意义是什么？](#二交叉熵损失函数写一下物理意义是什么)
  - [三、KL 散度与交叉熵的区别？](#三kl-散度与交叉熵的区别)
  - [四、多任务学习各loss差异过大怎样处理？](#四多任务学习各loss差异过大怎样处理)
  - [五、分类问题为什么用交叉熵损失函数不用均方误差（MSE）？](#五分类问题为什么用交叉熵损失函数不用均方误差mse)
  - [六、什么是信息增益？](#六什么是信息增益)
  - [七、多分类的分类损失函数(Softmax)？](#七多分类的分类损失函数softmax)
  - [八、softmax和交叉熵损失怎么计算，二值交叉熵呢？](#八softmax和交叉熵损失怎么计算二值交叉熵呢)
  - [九、如果softmax的e次方超过float的值了怎么办？](#九如果softmax的e次方超过float的值了怎么办)

## 一、介绍一下 KL 散度？

KL（Kullback-Leibler）散度衡量了两个概率分布之间的差异。其公式为：

![](img/微信截图_20240106220227.png)

## 二、交叉熵损失函数写一下，物理意义是什么？

交叉熵损失函数（Cross-Entropy Loss Function）是用于度量两个概率分布之间的差异的一种损失函数。在分类问题中，它通常用于衡量模型的预测分布与实际标签分布之间的差异。

![](img/微信截图_20240106223339.png)
...
![](img/微信截图_20240107092613.png)

多分类交叉熵：

![](img/微信截图_20240107092755.png)

二分类交叉熵：

![](img/微信截图_20240107092743.png)

## 九、如果softmax的e次方超过float的值了怎么办？

将分子分母同时除以 x 中的最大值，可以解决。

![](img/微信截图_20240107092824.png)
